...
class Expander:
    ...
    def maybe_expand(
        ...
        Args:
            ...
            predictor:          Frozen JEPA predictor (temporarily modified).
        ...
        best_chart = library[_argmin_umf(library, predictor,
                                         next_encoder_output, next_actions, motion_gate)]
        candidate = library.clone_from(library._charts.index(best_chart))
        _fit_candidate(candidate, predictor, self._deficit_chunks,
                       self.cfg.n_probe, self.cfg.lr)

        # ── Verify on the next unseen chunk ───────────────────────────────────
        cand_umf = compute_umf(candidate, predictor, next_encoder_output,
                                next_actions, motion_gate)
        best_umf = compute_umf(best_chart, predictor, next_encoder_output,
                                next_actions, motion_gate)
...
def _fit_candidate(
    ...
    Predictor is temporarily modified in-place; restored after fitting.
    """
