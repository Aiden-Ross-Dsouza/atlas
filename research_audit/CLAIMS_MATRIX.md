...
|---|---|---|---|---|
| **S-1** | The pre-2026-08-25 rollout bug is fully fixed, and every number produced after that date is trustworthy. Four specific defects named: 5x wrong time base, hard-zeroed proprio, wrong context window, unused correct implementation. | `CLAUDE.md` §0.1 | L0 | PENDING |
| **S-2** | All headless gates (G2, G3a, G3b, G5, G6) pass post-fix; G1 was rewritten and now passes; G4 is the only skipped gate. | `CLAUDE.md` §0.1, `HANDOFF.md` §7.2 | L0 | PENDING |
| **S-3** | `CLAUDE.md` §0.1's status section is accurate as of its own "last checked 2026-08-25" stamp. Known to be stale on at least G1 by `HANDOFF.md`'s own admission; extent of further staleness unknown. | `CLAUDE.md` §0.1 | L0 | PENDING |
| **S-4** | E0 is "closed" as a negative result and three independent rescue hypotheses were tested and rejected. | `E0_RECOVERY_PLAN.md` status banner | L0 | PENDING |
...
| **S-6** | `OPUS_REMAINING_TASKS.md` item 22 asserts that the 15pp bar E0 was judged against was **invented during the recovery process** and appears in neither the proposal nor the implementation plan, and sits close to the minimum detectable effect of its own N=20 sample. Also listed as an outstanding, un-applied correction. | `OPUS_REMAINING_TASKS.md` #22 | L0 | PENDING |
| **S-7** | `OPUS_REMAINING_TASKS.md` item 13 asserts E2 defines routing "correctness" by **regime label** (`correct_idx = 1` whenever regime != R0), not by demonstrated planning competence — which is in direct tension with the paper's own title, "Measure Fitness, Don't Infer the Regime". Also outstanding. | `OPUS_REMAINING_TASKS.md` #13 | L0 | PENDING |
| **S-8** | Two Claude Code sessions worked this repository in parallel on the same checkout, and some results were relayed between them rather than independently reproduced. | `HANDOFF.md` §0, §3, `ATLAS_SUMMARY.md` §6 | L0 | PENDING |
...
| S-2 (all headless gates pass) | **PARTIALLY FALSE** | G2 is vacuous (asserts nothing, §4.2) and G5 tests something different from what `CLAUDE.md` claims (§4.3) — both "pass" in the sense of not erroring, neither "passes" in the sense of having tested the property it's named for. G1, G3a, G3b, G6 hold as claimed. |
...
| S-6 (15pp bar invented, not in either design doc) | **Not independently re-derived this pass; nothing found that contradicts it.** Still PENDING full verification, but no counter-evidence emerged from four audit passes' worth of reading these documents. |
| S-7 (E2 correctness = regime label, not planning competence) | **Not independently re-derived this pass.** Consistent with the audit's broader finding that E2 has no planner in the loop (structural, confirmed) — the specific "correct_idx" definition claim itself was not re-traced to code this pass. |
| S-8 (two parallel sessions, relayed results) | Not independently checked this pass. |
