...
| **P10b** | **TRUST** | **The gate silently changes the denominator of the reported mean, and `n` is never recorded.** | `run_e0.py:466-472`: `losses` gets one entry per trajectory; `umf_scores` only per *ungated* trajectory — so `eval_loss` and `eval_umf` in the same `results.json` row can be means over different subsets, unmarked. `len(umf_scores)` is never printed or written (`:865-874`). Contrast `phase0_v3/p0c/p0c_it10_summary.json`, which **does** carry `"umf_episodes_with_value": 18` beside `"umf_mean_of_means"`. | Read the block against `score.py:86-89`. At 8 val trajectories a 10th-percentile gate is expected to drop ~0–1, so true `n` may be 7 or 8 and nothing records which. **Directional bias — CORRECTED 2026-08-29, was wrong.** This row originally claimed the gate drops low-displacement chunks, "which have the smallest UMF denominator and hence the largest UMF," making the gated mean systematically optimistic. That was reasoning from `umf`'s formula in isolation, never checked against data, and it does not hold: the numerator (prediction error) is not independent of displacement, and on real R2 on-policy chunks (`phase0_v3/p0g_smoke_v3/chunks_R2.jsonl`, n=35, the smoke's own collection) `corr(latent_disp, umf_c0) = +0.398` — higher-motion chunks tend to have **higher** UMF here, not lower. Concretely: the 10 chunks the trajectory-scale gate (244.2) drops beyond what the chunk-scale gate (142.3) drops have mean UMF 0.461, *below* the overall mean 0.553 — removing them **raises** the survivor mean, the opposite of "optimistic." **The correct, defensible claim is narrower: gating changes which subset the mean is taken over, in an unrecorded and direction-dependent way — direction is an empirical question per regime/collector, not a fixed sign to assume.** `n` not being recorded is the finding that stands; the assumed sign does not. Do not cite the old sign anywhere it was copied. If `everything` is gated, `nan` is formatted straight into the T5 table (`:909`) with no warning. |
